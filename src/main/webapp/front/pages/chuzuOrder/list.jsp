...
<div id="app">
    ...
    <div class="line-container">
        <p class="line" style="background: #EEEEEE;"> 我的房屋出租订单 </p>
    </div>
    <!-- 标题 -->
                ...
                <div class="layui-tab-content">
                    <table class="layui-table" lay-skin="nob">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            <td style="width: 60px;">{{item.chuzuName}}</td>
                            <td style="width: 80px;">{{item.huixingValue}}</td>
                            <td style="width: 60px;">{{item.chuzuMianji}}</td>
                            <td style="width: 100px;color: red">{{item.chuzuMoney}}</td>
                            <td style="width: 250px;">
                                <img :src="item.chuzuPhoto" style="width: 100px;height: 100px;object-fit: cover;">
                            </td>
                            <td style="width: 60px;">{{item.chuzuWeizhi}}</td>
                            <td style="width: 150px;">{{item.chuzuOrderDay}}</td>
                            <td style="width: 100px;color: red">{{item.chuzuOrderMoney}}</td>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 chuzuOrderContent 字段去除无用字符 如果字段长度大于60  就截取到60
            chuzuOrderContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            //列表查询
            ,pageList(chuzuOrderTypes) {
                this.chuzuOrderTypes = chuzuOrderTypes;
                // 获取列表数据
                layui.http.request('chuzuOrder/list', 'get', {
                    page: 1,
                    limit: this.limit,
                    chuzuOrderTypes: vue.chuzuOrderTypes
                }, function(res) {
                        ...
                        jump: function(obj, first) {
                            ...
                            if (!first) {
                                layui.http.request('chuzuOrder/list', 'get', {
                                    page: obj.curr,
                                    limit: obj.limit,
                                    chuzuOrderTypes: this.chuzuOrderTypes
                                }, function(res) {
            ...
            ,pay(id) {
                // 获取商品详情信息
                layui.http.requestJson(`chuzuOrder/pay`, 'PPST',{
                    id:id
                }, (res) => {
                    if(res.code==0){
                        layer.msg('支付成功', {
            ...
            ,refund(id) {
                // 获取商品详情信息
                layui.http.request(`chuzuOrder/refund?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('退款成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        // 获取列表数据
        http.request('chuzuOrder/list', 'get',{
            page: 1,
            limit: limit,
            chuzuOrderTypes: vue.chuzuOrderTypes
        }, function(res) {
                ...
                jump: function(obj, first) {
                    ...
                    if (!first) {
                        http.request('chuzuOrder/list', 'get', {
                            page: obj.curr,
                            limit: obj.limit,
                            chuzuOrderType: vue.chuzuOrderType
                        }, function(res) {
                            vue.dataList = res.data.list
                        })
                    }
