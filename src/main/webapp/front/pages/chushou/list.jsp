            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">房屋出售展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="chushouName" id="chushouName" placeholder="房屋名称" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                    ...
                    <ul>
                        <li v-for="(item,index) in dataList" v-bind:key="index">
                            <div class="ng-prslist-doc">
                                <div class="ng-prslist-img" @click="jump('../chushou/detail.jsp?id='+item.id)">
                                    <a href="#" class="ng-prsliik">
                                        <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.chushouPhoto?item.chushouPhoto.split(',')[0]:''" />
                                        </span>
                                ...
                                <div class="ng-prslist-tit" @click="jump('../chushou/detail.jsp?id='+item.id)"><span>{{item.chushouName}}</span></div>
                                <div class="ng-prslist-tit" v-if="item.chushouNewMoney"><span style="color: red;">{{item.chushouNewMoney}} RMB</span></div>
                            </div>
                        </li>
                    </ul>
                </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 chushouContent 字段去除无用字符 如果字段长度大于60  就截取到60
            chushouContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#chushouName').val() != null && jquery('#chushouName').val() != "" && jquery('#chushouName').val() != "null" ) {
                param['chushouName'] = jquery('#chushouName').val();
            }
         
            var huixingTypes = $("#huixingTypes").find("option:selected").val();
            if (huixingTypes != null && huixingTypes != "" && huixingTypes != "null") {
                param['huixingTypes'] = huixingTypes;
            }
            param['chushouTypes'] = '1';
                                                                                    // 获取列表数据
            http.request('chushou/list', 'get', param, function(res) {
    ...
    // 当前表的字典表
       ...
       function huixingTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=huixing_types", 'get', {}, function (res) {
               if(res.code == 0){
