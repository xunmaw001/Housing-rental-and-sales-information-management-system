            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑房东</li>
                    </ol>
                </div>
            </div>
        </div>
                            ...
                            <!-- 当前表的字段 -->
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <input id="fangdongIdNumber" name="fangdongIdNumber" class="form-control"
                                               onchange="fangdongIdNumberChickValue(this)"  placeholder="身份证号">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>手机号</label>
                                        <input id="fangdongPhone" name="fangdongPhone" class="form-control"
                                               onchange="fangdongPhoneChickValue(this)"  placeholder="手机号">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>照片</label>
                                        <img id="fangdongPhotoImg" src="" width="100" height="100">
                                        <input name="file" type="file" id="fangdongPhotoupload"
                                               class="form-control-file">
                                        <input name="fangdongPhoto" id="fangdongPhoto-input" type="hidden">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        <!-- 当前表的文件上传 -->
        $('#fangdongPhotoupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                var photoUrl= baseUrl + 'file/download?fileName=' + data.result.file;
                document.getElementById('fangdongPhotoImg').setAttribute('src',photoUrl);
                document.getElementById('fangdongPhoto-input').setAttribute('value',photoUrl);
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("fangdong/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addfangdong');
    ...
    //数字检查
        <!-- 当前表的数字检查 -->
        function fangdongIdNumberChickValue(e){
        ...
        function fangdongPhoneChickValue(e){
            var this_val = e.value || 0;
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                username: "required",
                password: "required",
                fangdongName: "required",
                sexTypes: "required",
                fangdongIdNumber: "required",
                fangdongPhone: "required",
                fangdongPhoto: "required",
            },
            messages: {
                username: "账户不能为空",
                password: "密码不能为空",
                fangdongName: "房东姓名不能为空",
                sexTypes: "性别不能为空",
                fangdongIdNumber: "身份证号不能为空",
                fangdongPhone: "手机号不能为空",
                fangdongPhoto: "照片不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addfangdong = window.sessionStorage.getItem("addfangdong");
        if (addfangdong != null && addfangdong != "" && addfangdong != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("fangdong/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
